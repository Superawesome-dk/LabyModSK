...
public class EffAddButtonWidget extends Effect {
    static {
        Skript.registerEffect(EffAddButtonWidget.class,
                "[LabyModSK] add button widget with id %integer% and %anchor% and offsetX %double% and offsetY %double% and value %string% and width %integer% and height %integer%[ and close on click %-boolean%] to [widgets of] %screen%"
        );
    }
    ...
    public boolean init(Expression<?>[] exprs, int matchedPattern, Kleenean isDelayed, SkriptParser.ParseResult parseResult) {
        ...
        exprCloseonclick = (Expression<Boolean>) exprs[7];
        exprScreen = (Expression<WidgetScreen>) exprs[8];
        return true;
    }
    ...
    protected void execute(Event e) {
        ...
        Boolean closeonclick = exprCloseonclick != null ? exprCloseonclick.getSingle(e) : false;
        WidgetScreen screen = exprScreen.getSingle(e);

        widget = new ButtonWidget(id, anchor, offsetX, offsetY, value, width, height);
        widget.setCloseScreenOnClick(closeonclick);
        screen.addWidget(widget);
    }

    @Override
    public String toString(Event e, boolean debug) {
        return "add button widget with id " + exprId.toString(e, debug) + " to widgets of " + exprScreen.toString(e, debug);
    }
}
