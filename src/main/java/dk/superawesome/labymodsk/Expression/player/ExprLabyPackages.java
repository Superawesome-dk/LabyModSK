...
public class ExprLabyPackages extends SimpleExpression<String> {
    ...
    protected String[] get(Event event) {
        ...
        if(LabyPlayer.isPresent()) {
            List<PackageExtension> Packages = LabyPlayer.get().getPackages();
            for (PackageExtension packageext : Packages) {
                JsonObject addonIndex = new JsonObject();
                addonIndex.addProperty("uuid", String.valueOf(packageext.getIdentifier()));
                addonIndex.addProperty("name", packageext.getName());
